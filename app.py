...
def index():
    if 'username' in session:
        ...
        total_upi = sum(transaction[2] for transaction in transactions
                        if transaction[6] == 'UPI')
        total_cash = sum(transaction[2] for transaction in transactions
                         if transaction[6] == 'Cash')
...
def add_transaction():
    if 'username' in session:
        ...
        c = conn.cursor()
        c.execute(
            "INSERT INTO transactions (user_id, date, category, "
...
def delete_transaction(transaction_id):
    if 'username' not in session:
        flash('You must be logged in to delete a transaction.')
        return redirect(url_for('login'))
    conn = sqlite3.connect(DATABASE)
    c = conn.cursor()
...
def statistics():
    ...
    if user_id:
        # Fetch data for statistics page for the logged-in user
        # from the database
        conn = sqlite3.connect(DATABASE)
        c = conn.cursor()
        ...
        total_expenses = (total_expenses_result[0]
                          if total_expenses_result else 0)
        ...
        expense_by_category = (
            dict(expense_by_category_result)
            if expense_by_category_result else {}
        )
        ...
        top_spending_categories = (
            ...
            if top_spending_categories_result else {}
        )
